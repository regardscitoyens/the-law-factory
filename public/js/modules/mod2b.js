...
function wrap(width) {
    d3.selectAll('text').each(function () {
        ...
        lines.push(line.join(" "));
        var devy = lineHeight * (lines.length - 1) / 2;
...
function init(data, step) {
    ...
    divs.forEach(function (f) {
        ...
        orderedGroupes.forEach(function (g) {
            var toAdd, filtered = gp.filter(function (k) {
                    return k.key.toLowerCase() === g.toLowerCase()
                }),
                curr = mydata.filter(function (e) {
...
sven.viz.streamkey = function () {
    ...
    streamkey.height = function (x) {
        // WTF return width but ot height ???
    ...
    streamkey.width = function (x) {
        // WTF return height but ot width ???
    ...
    streamkey.init = function () {
        ...
        layer.selectAll("path")
            ...
            .attr("d", function (d) {
                return drawLink(d[0], d[1], d[2], d[3])
            })
        ...
        for (j = 0; j < m; ++j) {
            ...
            for (i = 0; i < n; i++)
                stepsY[j].push({
                    ...
                    'index': i,
                    'x': data[i]['values'][j]['step'],
                    'color': data[i]['color'],
                    'speakers': data[i]['values'][j]['speakers'],
                    'category': data[i]['key'],
                    'label': data[i]['name']
                });
            var sorted = d3.nest().key(function (d) {
                return d[sorting_key]
            })
    ...
    var sortByTop = function (data) {
        return sort(data, "y", function (a, b) {
            if (a === "null") a = 0;
            if (b === "null") b = 0;
